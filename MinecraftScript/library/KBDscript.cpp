...
void clickLeft(int x, int y, const char* type);
...
void set_key_value() 
{
    ...
    GetWindowRect(FindWindow(NULL, (LPCSTR)"Minecraft"), &MSxy);
...
void clickLeft(int x, int y, const char* type) 
{
	GetWindowRect(GetForegroundWindow(), &MSxy);
...
void clickRight(int x, int y, const char* type) 
{
	GetWindowRect(GetForegroundWindow(), &MSxy);
	mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, (MSxy.left + x) * 65536 / rctScreen.right, (MSxy.top + y) * 65536 / rctScreen.bottom, 0, 0);
	Sleep(20);
...
void clickMove(int x, int y, const char* type) 
{
	GetWindowRect(GetForegroundWindow(), &MSxy);
	mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE, (MSxy.left + x) * 65536 / rctScreen.right, (MSxy.top + y) * 65536 / rctScreen.bottom, 0, 0);
}
