...
AUDIO::AUDIO(const char* fileName){
    ...
    if(d != string::npos) this->subFileName = this->fileName.substr(d+1);
}
...
AUDIO AUDIO::open(const char* fileName){
    ...
    FILE* audioFile = fopen(this->fileName, "rb");
    if(audioFile){
        if(this->subFileName == "mp3") fread(this->mp3H, 1, sizeof(this->mp3H), audioFile);
        else if(this->subFileName == "wav") fread(this->wavH, 1, sizeof(this->wavH), audioFile);
    }
    while(!feof(audioFile)){
        ...
        fread(buffer.begin(), sizeof(uint8_t) * BUFSIZE, 1 audioFile);
