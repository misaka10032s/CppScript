...
// std::cout<<"simMin: "<<NUM0<<", "<<NUM1<<", "<<mul<<", "<<exp<<", "<<"\n";
    ...
    if(!(NUM0.value.size() > digit)) NUM0.value.resize(digit + 1);
...
// std::cout<<"@: "<<NUM1point<<", "<<exp<<", "<<pointDiff<<", "<<modNUM0<<", "<<modNUM1<<", "<<size0<<", "<<size1<<", "<<pointOver0<<", "<<pointOver1<<", "<<"\n";
    for(int i=0; i<digit; i++){
        ...
        tmp = (long long int)((tmpNUM0 % modNUM0) * (DIGIT10 / modNUM0)) - (long long int)((tmpNUM1 % modNUM1) * (DIGIT10 / modNUM1) * mul) * (long long int)quick_pow10(exp%DIGITNUM) + nextSlotValue;
        nextNum = (tmp < 0 ? -(tmp) / DIGIT10 + 1 : 0);
        nextSlotValue = (tmpNUM0 / modNUM0) - (tmpNUM1 / modNUM1) * mul - nextNum;
// std::cout<<"i: "<<i<<", "<<tmpNUM0<<", "<<tmpNUM1<<", "<<mul<<", "<<nextSlotValue<<", "<<nextNum<<", "<<tmp<<"\n";
        NUM0.value[i] = tmp + (long long int)(nextNum * DIGIT10);
    }
}
...
// std::cout<<"@: "<<NUM1point<<", "<<exp<<", "<<pointDiff<<", "<<modNUM0<<", "<<modNUM1<<", "<<size0<<", "<<size1<<", "<<pointOver0<<", "<<pointOver1<<", "<<"\n";
    for(int i=digit-1; i>=0; i--){
        ...
        tmp = (long long int)((tmpNUM0 % modNUM0) * (DIGIT10 / modNUM0)) - (long long int)((tmpNUM1 % modNUM1) * (DIGIT10 / modNUM1)) * (long long int)quick_pow10(exp%DIGITNUM) + nextSlotValue;
        nextNum = (tmp < 0 ? -(tmp) / DIGIT10 + 1 : 0);
        nextSlotValue = (tmpNUM0 / modNUM0) - (tmpNUM1 / modNUM1) - nextNum;
// std::cout<<"i: "<<i<<", "<<tmpNUM0<<", "<<tmpNUM1<<", "<<mul<<", "<<nextSlotValue<<", "<<nextNum<<", "<<tmp<<"\n";
    }
...
// ================================================================================================
void BIGNUM::leftShift(DIGITDATATYPE NUM1, BIGNUM &res){
...
void BIGNUM::rightShift(DIGITDATATYPE NUM1, BIGNUM &res){
    if(NUM1 <= 0 || this->isInf) {
        res = *this;
        return;
    }
...
void BIGNUM::plus(BIGNUM const &NUM1, BIGNUM &res){
    ...
    for(int i=0; i<digit; i++){
        ...
        nextSlotValue = tmpNUM0 / modNUM0 + (tmpNUM1 / modNUM1 + (res.value[i] < 0)) * revMul;
        if(res.value[i] < 0) res.value[i] += DIGIT10;
        else res.value[i] %= DIGIT10;
    }
};
void BIGNUM::minus(BIGNUM const &NUM1, BIGNUM &res){
    bool signRev = this->sign != NUM1.sign;
    int revMul = signRev ? -1 : 1;
    int digit, tmpNUM0, tmpNUM1, nextSlotValue = 0;
    int size0, size1;
    ...
    if(compareAbs(*this, NUM1)){
        _NUM0 = &NUM1;
        _NUM1 = this;
        res.sign = signRev ? this->sign : !this->sign;
    }
    else{
        _NUM0 = this;
        _NUM1 = &NUM1;
...
void BIGNUM::multiply(BIGNUM const &NUM1, BIGNUM &res){
    int digit, size0, size1;
...
void BIGNUM::divideBy(BIGNUM const &NUM1, BIGNUM &res){
    BIGNUM NUM0;
    ...
    if((int)res.value.size() < std::max((nowDigit + digitExact - 1)/DIGITNUM + 1, 0)) res.value.resize(std::max((nowDigit + digitExact - 1)/DIGITNUM + 1, 0));
...
std::cout<<"\n/@@: "<<digitNUM0<<", "<<digitNUM1<<", "<<f0<<", "<<f1<<", "<<nowDigit<<", "<<"\n";
        ...
        // if(digitNUM0 < digitNUM1 || digitNUM0 == digitNUM1 && f0 < f1) { // NUM0 < NUM1
            // NUM1 >>= 1;
            digitNUM1--;
            nowDigit--;
            continue;
        ...
        f0 = firstDigit2(NUM0);
std::cout<<"compareAbs: "<<NUM0<<", "<<NUM1<<", "<< nowDigit-1<<", "<<compareAbs(NUM0, NUM1, nowDigit-1)<<"\n";
        if(!compareAbs(NUM0, NUM1, nowDigit-1)){ // NUM1 > NUM0 
        // if(digitNUM0 > digitNUM1 || digitNUM0 == digitNUM1 && f0 >= f1){ // NUM1 > NUM0
...
void BIGNUM::modulus(BIGNUM const &NUM1, BIGNUM &res){
