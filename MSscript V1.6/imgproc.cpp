...
void ScreenShot(std::vector<uint8_t>& Pixels, int& BitsPerPixel, int width, int height, int dx, int dy){
    Mat src;
    HWND hwnd = FindWindow(NULL, (LPCSTR)"MapleStory");
    RECT MSwh;
...
int GetEigenvalue2(vector<uint8_t>& Pixels, int width, int height, int BitsPerPixel){
    int bit = BitsPerPixel > 24 ? 4 : 3, e = 0;
    for (int j = 0; j < height; j++) {
        for(int i=0;i<width;i++){
            for(int k=0;k<3;k++){
                e += (Pixels[i * bit + j * width * bit + k] / 16) * 16;
            }
        }
    }
    return e;
